...
class LokFarmer:
    ...
    def __init__(self, token, captcha_solver_config):
        ...
        # Initialize march objects tracking
        ...
        # Handle skills activation
        skills_config = config.get('main', {}).get('skills', {})
        if skills_config.get('enabled', False):
            logger.info("Skills system is enabled, checking individual skills...")
            ...
            try:
                ...
                logger.error(f"Error in skills system initialization: {e}")
        else:
            logger.info("Skills system is disabled in config")
    ...
    def _reinitialize_bot(self):
        """Re-initialize the bot with stored token after API failures"""
        try:
            logger.info("Attempting to re-initialize bot with stored token...")
            ...
            logger.info("Found stored token, attempting re-authentication...")
            ...
            logger.info("Bot re-initialization successful!")
            return True

        except Exception as e:
            logger.error(f"Bot re-initialization failed: {str(e)}")
            return False
    ...
    def _validate_march_capacity_for_gathering(self, to_loc, final_check=False):
        ...
        try:
            ...
            # Use full march capacity as requested - no safety buffer needed
...
Expected End: {ended_time}"""
    ...
    def _prepare_march_troops(self, each_obj, march_type=MARCH_TYPE_GATHER):
        ...
        if march_type == MARCH_TYPE_MONSTER:
            ...
            # Check if monster level is in the range 3-8
            if monster_level < 3 or monster_level > 8:
                logger.info(
                    f'Monster level {monster_level} is outside the supported range (3-8)'
                )
                return []
            ...
            # Get fixed troops configuration for this specific monster level
            level_troops_config = normal_monsters_config.get('fixed_troops', {}).get(str(monster_level), {})
            if not level_troops_config:
                logger.info(f'No fixed troops configuration found for monster level {monster_level}')
                return []
            ...
            # Prepare march troops based on fixed configuration
            march_troops = []
            total_troops_needed = 0

            for troop_config in level_troops_config.get('troops', []):
                troop_code = troop_config.get('code')
                fixed_amount = troop_config.get('amount', 0)

                if fixed_amount <= 0:
                    continue

                available = available_troops.get(troop_code, 0)
                if available < fixed_amount:
                    logger.info(
                        f'Not enough troops of type {troop_code}: need {fixed_amount}, have {available}'
                    )
                    return []
                ...
                total_troops_needed += fixed_amount

            if not march_troops:
                logger.info('No troops configured for this monster level')
                return []

            logger.info(f'Prepared {total_troops_needed} total troops for monster level {monster_level}')
            return march_troops
    ...
    def _is_object_being_marched(self, target_loc):
        """Check if an object is already being marched to based on march/objects data"""
        try:
            # Check if we have recent march objects data (within last 2 minutes)
            if not self.march_objects_data or (time.time() - self.march_objects_last_update) > 120:
                logger.debug("No recent march objects data available for conflict detection")
                return False

            # Check different possible structures in march objects data
            march_objects = []

            # Try to find march objects in various possible fields
            if isinstance(self.march_objects_data, dict):
                # Check common field names for march data
                for field_name in ['objects', 'marches', 'data', 'items']:
                    if field_name in self.march_objects_data:
                        potential_objects = self.march_objects_data[field_name]
                        if isinstance(potential_objects, list):
                            march_objects.extend(potential_objects)
                            break
                        elif isinstance(potential_objects, dict):
                            march_objects.append(potential_objects)
                            break
            elif isinstance(self.march_objects_data, list):
                march_objects = self.march_objects_data

            # Check if target location is in any of the march objects
            target_loc_str = f"{target_loc[0]},{target_loc[1]},{target_loc[2]}"

            for march_obj in march_objects:
                if isinstance(march_obj, dict):
                    # Check various location field names
                    for loc_field in ['loc', 'toLoc', 'location', 'target_loc', 'destination']:
                        march_loc = march_obj.get(loc_field)
                        if march_loc:
                            # Handle different location formats
                            if isinstance(march_loc, list) and len(march_loc) >= 3:
                                march_loc_str = f"{march_loc[0]},{march_loc[1]},{march_loc[2]}"
                            elif isinstance(march_loc, str):
                                march_loc_str = march_loc
                            else:
                                continue

                            if march_loc_str == target_loc_str:
                                logger.info(f"CONFLICT DETECTED: Object at {target_loc} is already being marched to. Skipping gathering.")
                                return True

            logger.debug(f"No march conflict detected for location {target_loc}")
            return False

        except Exception as e:
            logger.error(f"Error checking march objects conflict: {e}")
            return False
    ...
    def _on_field_objects_gather(self, each_obj):
        ...
        # CHECK FOR MARCH CONFLICTS BEFORE PROCEEDING
        if self._is_object_being_marched(to_loc):
            logger.info(f"Skipping gathering at {to_loc} - object is already being marched to")
            return False
        ...
        try:
            ...
            logger.warning(f"Kingdom is not online for gathering, attempting to reconnect...")

            # Try simple kingdom reconnection
            if self._reconnect_kingdom():
                # Add a delay before retrying
                time.sleep(random.uniform(3, 6))

                try:
                    # Retry the gathering march
                    logger.info('Retrying gathering march after reconnection...')
                    self._start_march(to_loc, march_troops, MARCH_TYPE_GATHER)
                    return True
                except Exception as retry_error:
                    logger.error(f'Gathering march still failed after reconnection: {str(retry_error)}')
                    return False
            else:
                logger.error('Kingdom reconnection failed, cannot start gathering')
                return False
        except OtherException as error_code:
    ...
    def _on_field_objects_monster(self, each_obj):
        ...
        # Check if monster level is in supported range (3-8)
        if monster_level < 3 or monster_level > 8:
            logger.info(
                f'Monster level {monster_level} is outside supported range (3-8), skipping'
            )
            return False

        # Get fixed troops configuration for this monster level from normal_monsters config
        fixed_troops_config = normal_monsters_config.get('fixed_troops', {}).get(str(monster_level), {})
        if not fixed_troops_config:
            logger.info(f'No fixed troops configuration found for monster level {monster_level}')
            return False
        ...
        # Prepare march troops using fixed troops config
        march_troops = []
        total_troops_needed = 0

        for troop_config in fixed_troops_config.get('troops', []):
            troop_code = troop_config.get('code')
            fixed_amount = troop_config.get('amount', 0)
        ...
        try:
            ...
            logger.warning(f'Kingdom is not online for monster attack, attempting to reconnect...')
            ...
            if self._reconnect_kingdom():
                ...
                try:
                    ...
                    logger.info('Retrying monster attack after reconnection...')
                    self._start_march(monster_loc, march_troops, MARCH_TYPE_MONSTER)
                    return True
                except Exception as retry_error:
                    logger.error(f'Monster attack still failed after reconnection: {str(retry_error)}')
                    return False
            else:
                logger.error('Kingdom reconnection failed, cannot attack monster')
                return False
        except OtherException as error_code:
            error_str = str(error_code)
    ...
    def join_rally(self, rally_mo_id, battle_data=None):
        ...
        try:
            ...
            logger.debug(f"March queue validation - API numMarch: {current_marches}, Internal queue: {internal_queue_size}, Max allowed: {rally_max_marches}")
            ...
            try:
                ...
                if error_str == 'not_online':
                    logger.warning('Kingdom is not online for rally join, attempting to reconnect...')

                    # Try simple kingdom reconnection first
                    if self._reconnect_kingdom():
                        # Add a delay before retrying
                        time.sleep(random.uniform(3, 6))
                        ...
                        try:
                            ...
                            logger.info('Retrying rally join after reconnection...')
                            res = self.api.field_rally_join(rally_mo_id, march_troops)
                            logger.info(f'Successfully joined rally {rally_mo_id} after reconnection: {res}')
                        except Exception as retry_error:
                            logger.error(f'Rally join still failed after reconnection: {str(retry_error)}')
                            return False
                    else:
                        logger.error('Kingdom reconnection failed, cannot join rally')
                        return False
...
Rally ID: {rally_mo_id}"""
    ...
    def sock_thread(self):
        ...
        def on_buff_list(data):
            ...
            # Log currently active buffs to console
            if data:
                logger.info("=== ACTIVE BUFFS ===")
                for buff in data:
                    item_code = buff.get('param', {}).get('itemCode')
                    buff_type = "Unknown"

                    # Identify buff type based on item code
                    ...
                    remaining_time = buff.get('remainingTime', 'Unknown')
                    if remaining_time != 'Unknown' and isinstance(remaining_time, (int, float)):
                        hours = int(remaining_time // 3600)
                        minutes = int((remaining_time % 3600) // 60)
                        remaining_time = f"{hours}h {minutes}m"

                    logger.info(f"Active Buff: {buff_type} (Code: {item_code}) - Time Remaining: {remaining_time}")
                logger.info("==================")
            else:
                logger.info("No active buffs currently")

            # Update Golden Hammer status for building queue
            self.has_additional_building_queue = len([
                item for item in data if item.get('param', {}).get('itemCode')
                == ITEM_CODE_GOLDEN_HAMMER
            ]) > 0

            # Store active buffs for new buff management system
            self.active_buffs = data

            # OLD BUFF SYSTEM DISABLED - Now handled by _buff_management_thread
    ...
    def _socf_thread_internal(self, radius, targets, share_to=None):
        ...
        try:
            ...
            def on_march_objects(data):
                ...
                logger.info(f'MARCH OBJECTS ENDPOINT: Raw data received at {current_time}: {data}')

                # Store the march objects data for later comparison
                try:
                    packs = data.get('packs')
                    if packs:
                        logger.info(f'[{current_time}] MARCH OBJECTS - Processing packs field (length: {len(packs)})')
                        gzip_decompress = gzip.decompress(bytearray(packs))
                        logger.info(f'[{current_time}] MARCH OBJECTS - Decompressed size: {len(gzip_decompress)}')
                        data_decoded = self.api.b64xor_dec(gzip_decompress)
                        logger.info(f'[{current_time}] MARCH OBJECTS DECODED: {data_decoded}')

                        # Store the decoded march objects data
                        self.march_objects_data = data_decoded
                        self.march_objects_last_update = time.time()

                        # Analyze decoded data structure
                        if isinstance(data_decoded, dict):
                            logger.info(f'[{current_time}] MARCH OBJECTS DECODED - Keys: {list(data_decoded.keys())}')
                            for key, value in data_decoded.items():
                                if isinstance(value, list):
                                    logger.info(f'[{current_time}] MARCH OBJECTS DECODED - {key}: list with {len(value)} items')
                                    if value and len(value) > 0:
                                        logger.info(f'[{current_time}] MARCH OBJECTS DECODED - {key} first item: {value[0]}')
                                else:
                                    logger.info(f'[{current_time}] MARCH OBJECTS DECODED - {key}: {type(value).__name__} = {value}')
                    else:
                        logger.info(f'[{current_time}] MARCH OBJECTS (no packs field): {data}')
                        # Store direct data if no packs field
                        self.march_objects_data = data
                        self.march_objects_last_update = time.time()

                        # Check if there are march objects directly in the data
                        if 'objects' in data:
                            objects = data['objects']
                            logger.info(f'[{current_time}] MARCH OBJECTS - Direct objects field found: {len(objects) if hasattr(objects, "__len__") else "N/A"} items')
                            if isinstance(objects, list) and objects:
                                logger.info(f'[{current_time}] MARCH OBJECTS - First object sample: {objects[0]}')

                except Exception as e:
                    logger.error(f'Error processing march objects data: {e}')
                    logger.info(f'[{current_time}] MARCH OBJECTS ERROR: {e} - Raw data: {data}')
                    import traceback
                    logger.info(f'[{current_time}] MARCH OBJECTS ERROR TRACEBACK: {traceback.format_exc()}')
            ...
            def on_field_objects(data):
                ...
                current_time = arrow.now().format('HH:mm:ss.SSS')
                logger.info(f'[{current_time}] FIELD OBJECTS SCAN: Processing {len(objects)} objects from field')
                ...
                for each_obj in objects:
                    ...
                    # Skip if object is not in valid state (no logging for these)
                    if state != 1:
                        continue

                    # Check if monster code matches our targets (no logging for non-matches)
                    if code not in target_code_set:
                        continue

                    # Only log when we find a targeted object
                    logger.debug(f'[{current_time}] SCANNING OBJECT: code={code}, level={level}, loc={loc}, state={state}')
                    ...
                    if not allowed_levels or level in allowed_levels:
                        ...
                        if code in [
                            ...
                            try:
                                ...
                                current_marches = march_info.get('numMarch', 0)
                                rally_start_max_marches = config.get('rally', {}).get('start', {}).get('numMarch', 8)

                                # Check if march queue has available slots with comprehensive validation
                                if current_marches >= rally_start_max_marches:
                                    logger.info(
                                        f'March queue full from numMarch ({current_marches}/{rally_start_max_marches}), skipping rally start'
                                    )
                                    continue

                                # Also check internal troop_queue limit
                                if len(self.troop_queue) >= rally_start_max_marches:
                                    logger.info(
                                        f'March queue full from troop_queue ({len(self.troop_queue)}/{rally_start_max_marches}), skipping rally start'
                                    )
                                    continue
                    ...
                    if code in OBJECT_MINE_CODE_LIST:
                        ...
                        if enable_gathering and code in target_code_set:
                            logger.info(f"[{current_time}] TARGET FOUND FOR GATHERING: Resource {code} level {level} at {loc}")
                            logger.info(f"[{current_time}] CHECKING MARCH CONFLICTS: Validating if object at {loc} is already being marched to...")
                            result = self._on_field_objects_gather(each_obj)
                            if result:
                                logger.info(f"[{current_time}] GATHERING STARTED: Successfully initiated gathering march to {loc}")
                            else:
                                logger.info(f"[{current_time}] GATHERING SKIPPED: March not started (conflict detected or other constraint)")
                        else:
                        ...
                        if enable_monster_attack and code in target_code_set:
                            logger.info(f"[{current_time}] TARGET FOUND FOR ATTACK: Monster {code} level {level} at {loc}")
...
Status: {status}"""
    ...
    def _get_current_active_buffs(self):
        """Get current active buffs with enhanced validation"""
        try:
            # Try to get fresh buff data from the kingdom enter response
            ...
            # Fallback to stored active_buffs
            return getattr(self, 'active_buffs', [])

        except Exception as e:
            logger.debug(f"Failed to get fresh buff data: {e}")
            return getattr(self, 'active_buffs', [])
    ...
    def _buff_management_thread(self):
        ...
        while True:
            try:
                ...
                logger.info("Checking buff status and reactivating if needed...")
                ...
                # Get fresh active buffs data with enhanced validation
