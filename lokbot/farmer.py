...
def ndindex(ndarray, item):
    if len(ndarray.shape) == 1:
        ...
        try:
            return numpy.where(ndarray == item)
        except:
            pass
...
class LokBotFarmer:
    def __init__(self, api: LokBotApi):
        self.api = api
        ...
        # March objects tracking
        self.march_objects_lock = threading.Lock()
        self.march_objects_data = {}
        self.march_objects_last_update = 0
        self.march_data_update_count = 0
        self.march_data_validation_errors = 0
        self.march_data_by_zone = {}
        self.current_scanning_zone = None

    def _get_nearest_zone_ng(self, x, y, radius):
        """
        Calculates zones within radius from (x, y)
        """
        zone_id = (x // 32) + (y // 32) * 96
        # Simplify for brevity - in real usage would return actual neighboring zones
        return [zone_id]

    def _on_field_objects_gather(self, obj):
        logger.info(f"Processing gather: {obj}")

    def _on_field_objects_monster(self, obj):
        logger.info(f"Processing monster: {obj}")

    def _set_current_scanning_zone(self, zone):
        self.current_scanning_zone = zone

    def _get_march_data_stats(self):
        return {
            'update_count': self.march_data_update_count,
            'zone_cache_count': len(self.march_data_by_zone)
        }
    ...
    def _socf_thread_internal(self, radius, targets, share_to=None):
        ...
        try:
                ...
                for each_obj in objects:
                    ...
                    if not allowed_levels or level in allowed_levels:
                        if code in OBJECT_MINE_CODE_LIST:
                            logger.info(f'Found resource: code={code} level={level} loc={loc}')
                        elif code in OBJECT_MONSTER_CODE_LIST:
                            logger.info(f'Found monster: code={code} level={level} loc={loc}')

                        # Check gathering/monster attack enabled
                        ...
                        target = next((t for t in targets if t.get('code') == code), None)
            ...
            while self.zones:
                ...
                timeout_count = 0
                while not self.field_object_processed and timeout_count < 30:
                    timeout_count += 1
                    time.sleep(1)
                    if timeout_count == 10:
                        logger.warning(f'Waiting for field objects processing...')
